...
<script>
	...
	function list(page) {
		...
		location.href = "${path}/board/board_list.do?search_option="
				+ search_option + "&keyword=" + keyword + "&curPage=" + page;
	}
</script>
			...
			<div class="table-responsive">
				...
				<div align="center">
					<form name="form1" method="get"
						action="${path}/board/board_list.do">
						<select class="form-select" name="search_option"
							style="width: 10%; flaot: left; display: inline-block; margin: 0 auto;">
							<option value="member_id"
								<c:if test="${map.search_option == 'member_id'}">selected</c:if>>이름</option>
							<option value="title"
						...
						</select> <input name="keyword" value="${map.keyword}" class="form-control"
							style="width: 20%; display: inline-block;"> <input
							type="submit" value="조회" class="btn btn-secondary">
					</form>
				</div>
					...
					<c:choose>
						...
						<c:when test="${!empty list_view }">
							<c:forEach var="row" items="${list_view}">
								<tr>
									<td>${row.id}</td>
									<td><a href="${path}/board/board_view.do/${row.id}">${row.title}</a>
										<span style="color: red;">(${row.comment_cnt})</span></td>
									<td>${row.member_id}</td>
